...
@Component({
    selector: 'jhi-account-transactions',
...
currentAccount: any;
    ...
    loadAll() {
        this.accountTransactionsService.query({
            page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/account-transactions'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: AccountTransactions) {
        return item.id;
    }
    registerChangeInAccountTransactions() {
        this.eventSubscriber = this.eventManager.subscribe('accountTransactionsListModification', (response) => this.loadAll());
    }
